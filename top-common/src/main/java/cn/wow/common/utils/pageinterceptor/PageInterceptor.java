...
public class PageInterceptor implements Interceptor {
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		if (obj instanceof Page<?>) {
			Page<?> page = (Page<?>) obj;
			if(0 ==page.getStart() || 0== page.getPageSize()) {
		...
		if (obj instanceof PageMap<?,?>) {
			PageMap<String,Object> paramMap = (PageMap<String,Object>) obj;
			Object pPageNum = paramMap.get("pageNum");
			if(null ==pPageNum || null==paramMap.get("pageSize")) {
				return invocation.proceed(); 
			}
			// 通过反射获取delegate父类BaseStatementHandler的mappedStatement属性
			MappedStatement mappedStatement = (MappedStatement) ReflectUtil.getFieldValue(delegate, "mappedStatement");
			...
			String sql = boundSql.getSql();
			// 给当前的page参数对象设置总记录数
			this.setTotalRecordMap(paramMap, mappedStatement, connection);
			// 获取分页Sql语句
	 ...
	 * @return
	 */
	private String getPageSql(PageMap<String,Object> param, String sql) {
		StringBuffer sqlBuffer = new StringBuffer(sql);
		
		if ("mysql".equalsIgnoreCase(databaseType)) {
			return getMysqlPageSql(null, sqlBuffer);
		} else if ("oracle".equalsIgnoreCase(databaseType)) {
			return getOraclePageSql(param, sqlBuffer);
		}
		return sqlBuffer.toString();
	}
	 ...
	 * @return Oracle数据库的分页查询语句
	 */
	private String getOraclePageSql(PageMap<String,Object> paramMap, StringBuffer sqlBuffer) {
		// 计算第一条记录的位置，Oracle分页是通过rownum进行的，而rownum是从1开始的
		int offset = (Integer.valueOf(paramMap.get("pageNum").toString()) - 1) * Integer.valueOf(paramMap.get("pageSize").toString()) + 1;
		sqlBuffer.insert(0, "select u.*, rownum r from (").append(") u where rownum < ")
				.append(offset + Integer.valueOf(paramMap.get("pageSize").toString()));
		sqlBuffer.insert(0, "select * from (").append(") where r >= ").append(offset);
		// 上面的Sql语句拼接之后大概是这个样子：
		// select * from (select u.*, rownum r from (select * from t_user) u
		// where rownum < 31) where r >= 16
	 ...
	 *            当前的数据库连接
	 */
	private void setTotalRecordMap(PageMap<String,Object> paramMap, MappedStatement mappedStatement, Connection connection) {
		...
		try {
			...
			if (rs.next()) {
				...
				// 给当前的参数page对象设置总记录数
				paramMap.put("totalRecord", totalRecord);
			}
